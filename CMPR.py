...
def tpl_decompress(file, offset):
    ...
    for i in image_headers:
        ...
        sizex = int.from_bytes(file.read(0x02), "big")
        sizey = int.from_bytes(file.read(0x02), "big"); file.read(0x03)
        mode = file.read(0x01)
                    ...
                    for y in range(8):
                        for ymod in range(4):
                            ...
                            for x in range(2):
                                ...
                                image[(ycblock*8 + y)][xcblock*8 + (x+ymod*2)] = (value, value, value)
                    ...
                    for yblock in range(2):
                        for xblock in range(2):
                            ...
                            for y in range(4):
                                indices = int.from_bytes(file.read(0x01), "big")
                                for x in range(4):
                                    image[(ycblock*8 + yblock*4 + y)][xcblock*8 + xblock*4 + (3-x)] = palette[indices >> (x * 2) & 3]
...
if __name__ == "__main__":
    displayrgb(tpl_decompress(open(path, "rb"), offset)[0])