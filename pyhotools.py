...
class ho_archive:
    ...
    def get_game(self):
        with open(self.path, "rb") as file:
            ...
            game = ""
            for i in range(4):
                game += chr(int.from_bytes(file.read(0x01), "big"))
                file.read(0x01)
    ...
    def save_archive(self):
        ...
        with open(self.path, "rb") as file:
            for i in self.simple_objects:
                ...
                bytes_to_write += castsimpleobject.save(file, offset, rotation_data, position_data, scale_data, mass_data, friction_data, i, 0x10*(self.game == "WALE"))
    ...
    def get_models(self):
        with open(self.path, "rb") as file:
            ...
            for si, simp in enumerate(self.simple_objects):
                ...
                if simp.alternatemodel != "": continue
                ...
                for ii, i in enumerate(iteration):
                    if i[4] == simp.model_id:
                        ...
                        file.read(0x30  + 0x28*(self.game != "WALE")+ float_amount*0x30) #file.read(0x58 + float_amount*0x30)
                ...
                for mi, model_header_id in enumerate(model_header_ids):
                    ...
                    material_text_id = 0
                    texture = None # Root
                    material_id = b"\x00\x00\x00\x00\x00\x00\x00\x00"
                    break_flag = False
                    for ii, i in enumerate(iteration): # Now "i" will be a single entry
                        #print(i, model_header_id)
                    ...
                    for ii, i in enumerate(iteration): # Now "i" will be a single entry
                        if i[4] == generic_shader:
                            file.seek(i[1])
                            file.read(0x87 + 0x10*(self.game != "WALE"))
                            size_divisor = []
        ...
        with open(self.path, "rb") as file:
            for xy, x in enumerate(self.section_headers):
                for yi, y in enumerate(x.child.layers): # Now "y" will be a table
                    for ii, i in enumerate(y.entries): # Now "i" will be a single entry
                        file.seek(i[1])
                        temp = castsimpleobject.cast(file, i[5], xy, yi, ii, i, 0x10*(self.game == "WALE"))
