...
def update_visibility():
    ...
    for model in models:
        ...
        if model.type_ == b"\xA8\x83\xD6\xF4":
...
def init():
    ...
    geo_visible_text = urs.Text(text="Non-geometry Objects: ", position=(0.61, 0, 0), origin=(0.5, 1), font=r"Krabby Patty.ttf")
...
def view_model(verts_in, tris, texture, texture_ids, uvs_in, position, rotation, scale, mass, friction, index, size_divisor, anim_id, type_, name, model_id, alternatemodel):
    global models
    try:
        ...
        if alternatemodel != "":
            ...
            collider = "mesh"
            curr_model = modelclass(model = alternatemodel, texture_ = texture, position = position, scale = scale, rotation = rotation, mass = mass, friction = friction, index = index, anim_id = anim_id, type_ = type_, name = name, model_id = model_id, collider=collider, islight=True)
        ...
        #collider = [mesh, "sphere"][len(temp_verts) > 2000]
        collider = "mesh"
        curr_model = modelclass(model = mesh, texture_ = texture, position = position, scale = scale, rotation = rotation, mass = mass, friction = friction, index = index, anim_id = anim_id, type_ = type_, name = name, model_id = model_id, collider=collider, islight=False)
        # -----
...
def view_scene(archive):
    ...
    for index in range(0, len(archive.simple_objects)):
        ...
        for x in range(len(texture)):
            if texture[x] != [None] and texture[x] != ["INVALID_TYPE"]:
                #print(texture[x])
